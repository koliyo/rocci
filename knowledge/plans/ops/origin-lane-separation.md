...
`staging` and `production` no longer publish the same stack. `/srv/rocci` is
only the deploy-owned parent. Production is `/srv/rocci/prod` on `:8080`
without live-example containers. Staging is `/srv/rocci/staging` on `:8081`
with its own SQLite volumes and live apps.[^lanes][^prod-readme]
...
| Production quality | `/srv/rocci/prod` is production (hybrid only). Leftover `current` / `releases` under `/srv/rocci` and `/srv/rocci-staging` are not lane roots.[^prod-readme] |
...
**Bound:** Origin README (lane table, leftovers, migrate-then-promote order)
and Tunnel ingress example.[^prod-readme][^ingress]

**Status:** implemented in this revision. README rewritten 2026-08-30 for
the nested `/srv/rocci/{prod,staging}` tree.
...
[^prod-readme]: Parent vs lane roots, leftover `/srv/rocci-staging`, migrate-then-Tunnel-then-promote, smoke curls on `:8080` and `:8081`.
