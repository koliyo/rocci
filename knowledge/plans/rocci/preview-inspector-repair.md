...
sources:
  ...
  - id: research
    resource: ../../research/rocci/preview-inspector.md
...
| `rocci run` window | `examples/rocci/standalone/counter` | pass — sibling origin; product origin `/__rocci/dev` 404 | **fail** F1 on Roc/HTML | same overlay | Counter app, not docs chrome | **fail** sibling logs `[]` | pass — `/` all four views; HTML captured with window |
| `rocci run` `--no-window` | `examples/rocci/standalone/counter` | **fail** — no `InspectorServer` | blocked | blocked (no overlay) | n/a | blocked | blocked |
