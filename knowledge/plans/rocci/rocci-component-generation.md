...
record](/research/rocci/rocci-components-in-generation.md).[^research][^catalog-shell]
...
classified as base Rocci in `rocci-ops/src/rocci_ops/workspace_deps.py`.[^deps-check]
...
`rocci-ops/src/rocci_ops/workspace_deps.py`, owns cache lookup, native spawn, and an
...
- Cache tests cover hash identity, fingerprint skip, mtime-lie rehash,
  ...
  invoking Roc.
- `rocci-ops/src/rocci_ops/workspace_deps.py` still forbids OKF → Rocdown.
