...
* [A Roc port of rocci-ops is a parallel exercise](rocci-ops-roc.md) - Rewrite `rocci-ops` in ordinary Roc on branch `rocci-ops-roc` to test command parity and basic-cli viability. Python plus uv stays the operator CLI. Phase 7 table: harness match and origin x64musl are go; CI uv replacement and cutover are no-go. Implementation plan: [Roc rocci-ops](/plans/ops/rocci-ops-roc.md). Exploratory.
