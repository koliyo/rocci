...
* [A Roc port of rocci-ops is a parallel exercise](rocci-ops-roc.md) - Rewrite `rocci-ops` in ordinary Roc on branch `rocci-ops-roc` to test command parity and basic-cli viability. Python plus uv stays the operator CLI. Phase 0 pin recorded (`UnixBytes` argv, `exec_exit_code!` `Ok` codes, `packages` key rewrite). Implementation plan: [Roc rocci-ops](/plans/ops/rocci-ops-roc.md). Exploratory.
