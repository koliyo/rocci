...
- **Do not implement** the parser unless the user asks. Example files will not
  ...
  `knowledge/plans/rocdown/generalized-rocdown-block-model.md`.[^impl-plan]
- **Read next:** this record, then
  ...
  [implementation plan](/plans/rocdown/generalized-rocdown-block-model.md). The
...
by the later [ungram AST research](/research/rocci/ungram-ast.md).[^ungram-research]

## Recommended next experiments

Delivery phases live in
[the implementation plan](/plans/rocdown/generalized-rocdown-block-model.md).
...
- If ungram becomes canonical, who fails CI when it drifts from `ast.rs`?
  Follow-on: [ungram AST research](/research/rocci/ungram-ast.md) recommends
