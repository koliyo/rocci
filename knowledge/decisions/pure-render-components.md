...
A colocated component file can be mistaken for a runtime component instance that owns state, effects, or lifecycle. Rocci instead compiles markup into ordinary Roc functions and keeps application orchestration separate.
...
Persistence, request routing, decoding, effects, sessions, and durable domain state stay outside template lowering. The same pure renderer can be called for a full page or for a server-produced fragment.

## Consequences

Components remain composable Roc code with no hidden instance lifecycle. State architecture can evolve independently, and testing can compare rendered values without starting the host. Authors must pass required state explicitly and use application handlers for mutation.
