...
2. Understand the repository's GitHub Actions workflows in `.github/workflows/`:
   ...
   - `cut-release.yml`: `workflow_dispatch` only. Runs `rocci-ops release`
     (wait for existing `ci.yml` on the source SHA → version commit → tag).
     Does not start CI and does not wait for `knowledge.yml`. Not attached
     to the `release`, `staging`, or `production` environments.
...
With no argument, that lists open PRs via `gh`. With a number, URL, or
branch, it fetches the tip and switches this checkout to `pr/<head-branch>`.
