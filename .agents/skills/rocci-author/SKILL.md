...
- **`backend/` vs `ui/`**: Split an advanced standalone app so `backend/` owns `@context` / `@init` / routes / SQLite plus ordinary `*.roc` rules, and `ui/` owns pure `@component` markup, fixtures, and `@css`. `examples/rocci/standalone/blocks/` is the current advanced example. Put `rocci.toml` at the **app root** (the directory that contains both folders). Optional `[app].entry` names the primary `.rocci` relative to that file. `rocci run <dir>` or `rocci run File.rocci` walks up to the app root; a repository-root `rocci.toml` is not an app. At most one process `@init` / `@context` pair per app.
...
4. Treat `test/AllSyntax.rocci` as a compiler fixture, not as a style guide.
   ...
   and the pinned Roc nightly (`nightly-2026-09-03-62fcb65`): snake_case
