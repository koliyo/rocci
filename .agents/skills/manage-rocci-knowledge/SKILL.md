...
description: Query, inspect, validate, author, or review Rocci's `knowledge/` Open Knowledge Format bundle with `rocs knowledge`. Use for architecture and decision retrieval, knowledge-record edits, lifecycle or provenance review, graph inspection, and OKF diagnostics. Do not use for ordinary source-code changes unless they also require consulting or updating canonical knowledge.
---

# Manage Rocci Knowledge

Use the checked-in `knowledge/` bundle as the canonical database and the
repository's `rocs knowledge` CLI as its deterministic interface. Keep domain
...
```sh
cargo test -p rocs okf::
cargo test -p rocs-cli knowledge
```
