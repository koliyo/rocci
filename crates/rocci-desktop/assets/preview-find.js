(function () {
  if (!window.__rocciPreviewNav || window.__rocciPreviewNav.find) {
    return;
  }
  const SKIP_TAGS = {
    SCRIPT: true,
  ...
  const SKIP_HOSTS = {
    "ROCCI-PREVIEW-NAV": true,
    "ROCCI-PREVIEW-DEV": true,
    "ROCCI-PREVIEW-FIND": true,
    "ROCCI-PREVIEW-GOTO": true,
  };
  const canHighlight =
    typeof CSS !== "undefined" && CSS.highlights && typeof Highlight === "function";
  const host = document.createElement("rocci-preview-find");
  const shadow = host.attachShadow({ mode: "open" });
  const sheet = document.createElement("style");
  sheet.textContent = __ROCCI_PREVIEW_FIND_CSS__;
  const tpl = document.createElement("template");
  tpl.innerHTML = __ROCCI_PREVIEW_FIND_HTML__;
  ...
  const clearMarks = () => {
    const marks = document.querySelectorAll("mark.rocci-find-mark");
  ...
  const clearHighlights = () => {
    if (canHighlight) {
      CSS.highlights.delete("rocci-find");
      CSS.highlights.delete("rocci-find-current");
    }
    clearMarks();
  };
  ...
  const reveal = () => {
    ...
    if (canHighlight) {
      ...
      CSS.highlights.delete("rocci-find-current");
      if (rest.length) {
        CSS.highlights.set("rocci-find", new Highlight(...rest));
      }
      CSS.highlights.set("rocci-find-current", new Highlight(current));
    } else {
      clearMarks();
      for (let i = matches.length - 1; i >= 0; i--) {
        const mark = document.createElement("mark");
        mark.className = i === index ? "rocci-find-mark current" : "rocci-find-mark";
        try {
