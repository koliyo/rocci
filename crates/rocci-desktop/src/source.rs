...
fn candidates(spec: &str) -> Vec<String> {
    ...
    if trimmed.is_empty() {
        return vec!["index.md".into(), "index.rocdown".into()];
    }
    vec![
        format!("{trimmed}.md"),
        format!("{trimmed}.rocdown"),
        format!("{trimmed}/index.md"),
        format!("{trimmed}/index.rocdown"),
    ]
}
...
mod tests {
    ...
    fn resolves_catalog_paths_and_routes() {
        ...
        assert!(
            resolve_source_file(&root, "guide.rocdown")
                .unwrap()
                .ends_with("guide.rocdown")
        );
