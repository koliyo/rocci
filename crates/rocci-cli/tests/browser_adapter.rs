...
fn rocci_bin() -> PathBuf {
    let mut path = env::current_exe()
        ...
        .join("rocci");
    if !path.is_file() {
        path = PathBuf::from(env!("CARGO_BIN_EXE_rocci"));
    }
    path
}
...
fn probe_and_list_over_stdio() {
    ...
    let mut child = Command::new(rocci_bin())
        .arg("browser-adapter")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    assert_eq!(init["result"]["adapterId"], "rocci");
    ...
    assert_eq!(listed["result"]["documents"][0]["id"], "App.rocci");
