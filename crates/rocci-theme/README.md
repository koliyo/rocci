...
1. `@page { theme: "..." }`
2. `rocci run --theme ...` (also `build` / `inspect`)
3. `ROCCI_THEME`
4. builtin `paper`
...
```sh
rocci run --theme paper foo.rocdown
rocci run --theme path/to/theme.css foo.rocdown
rocci run --color-scheme dark foo.rocdown
```
