...
Html := [].{
	...
	## Generic attribute for generated markup (`Html.attribute("class", x)`).
	## Hand-written Roc can keep using `Attribute.class` and friends.
	attribute = Attribute.attribute

	## HTML boolean attribute. Presence is true; generated templates pass `True`.
	...
	## Empty node. Generated templates use this for missing children.
	empty = text("")
	...
	## Render sibling nodes as the nominal `Fragment` string type.
	render_fragment : List(HtmlNode) -> Fragment
	render_fragment = |nodes| Fragment(render_children(nodes))

	## Sibling nodes as one `Node`. Generated templates emit forests this way.
	## Distinct from [`render_fragment`](#Html.render_fragment).
	fragment = |nodes|
