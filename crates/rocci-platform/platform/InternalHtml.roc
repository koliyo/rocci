...
## The stable internal representation behind [`InternalHtml.Node`](#InternalHtml.Node).
HtmlNode := [
	Text(Str),
	Raw(Str),
...
##
## ```roc
## page = InternalHtml.html([], [
##     InternalHtml.body([], [
##         InternalHtml.h1([], [InternalHtml.text("Hello <Roc>")]),
##         InternalHtml.a([Attribute.href("/account")], [InternalHtml.text("Account")]),
##     ]),
## ])
##
## response = Response.from_status(200)
##     .with_headers([{ name: "Content-Type", value: "text/html; charset=utf-8" }])
##     .with_body(Str.to_utf8(InternalHtml.render(page)))
## ```
InternalHtml := [].{
...
render_children = |children| Str.join_with(children.map(InternalHtml.render_without_doc_type), "")
...
expect {
	node = InternalHtml.div(
		[Attribute.class("\"<&'\n\r")],
		[InternalHtml.text("<Roc & friends>")],
	)

	InternalHtml.render(node)
		== "<!DOCTYPE html><div class=\"&quot;&lt;&amp;&#39;&#10;&#13;\">&lt;Roc &amp; friends&gt;</div>"
}

expect {
	fragment = InternalHtml.render_fragment([
		InternalHtml.p([Attribute.id("first")], [InternalHtml.text("one < two")]),
		InternalHtml.p([], [InternalHtml.text("three")]),
	])
...
expect {
	document = InternalHtml.render_document(InternalHtml.html([], [InternalHtml.body([], [InternalHtml.text("Hello")])]))
	document.to_str() == "<!DOCTYPE html><html><body>Hello</body></html>"
}

## Trusted raw HTML is the explicit escape hatch.
expect {
	InternalHtml.render_without_doc_type(
		InternalHtml.div([], [InternalHtml.dangerously_include_unescaped_html("<strong>trusted</strong>")]),
