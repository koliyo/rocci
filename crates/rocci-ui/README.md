...
4. **Shared client chrome** (`assets/toc.js`, `assets/goto.js`, `assets/copy.js`): small page-owned scripts. `goto.js` is the Cmd/Ctrl-K fuzzy page palette and same-origin HTML swap used by Rocdown, rocci.dev, OKF review, and desktop preview. `copy.js` adds a copy-to-clipboard control on fenced code blocks; it ships concatenated into the hashed `goto.js` payload.
