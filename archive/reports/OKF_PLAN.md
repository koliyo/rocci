...
| 1 — parser and three-record vertical slice | Complete (2026-08-16) | Parser, profile validation, footnotes, reserved files, JSON inspection, knowledge CLI/build, three seed records, and the full workspace validation are complete |
...
| Field | Policy in the strict profile |
| --- | --- |
| `type` | Required by OKF; must use the controlled type list below during bootstrap |
| `title` | Required by the strict profile; one clear noun phrase |
...
- `rocs::okf` discovers strict `.md` bundles, parses YAML to a normalized metadata map without discarding extension fields, separates base OKF from the strict profile, recognizes `index.md` and `log.md`, rejects executable declarations/raw HTML, validates keyed sources, and resolves bundle-root concept links.
...
- The real bundle passes validation with lifecycle/provenance warnings but no errors; concept and graph inspection and two identical builds succeed. Focused OKF/body-parser tests, `cargo check --workspace`, the full `cargo test --workspace`, `rocs check docs`, and existing Rocdown tests pass.

Exit validation completed on 2026-08-16 after `cargo test --workspace` passed, including `build::tests::session_reuses_apply_binary_when_roc_sources_are_unchanged`. The earlier hang is retained here as historical investigation context, not a current blocker.
...
- strict-profile validation now emits `OKF4004` for stale records, `OKF4005` when human verification predates generation, `OKF4006` when a tracked source has a newer git commit, `OKF4007` when a local source is untracked, and `OKF4008` when tracked evidence has uncommitted changes. Timestamp comparison normalizes RFC 3339 offsets.
...
- Knowledge builds now emit deterministic `catalog.json`, heading-chunked `search.json`, `llms.txt`, `validation.json`, and rendered HTML.
- Catalog inspection and search support type, tag, lifecycle status, authority, derived trust-tier, and stale filters. Search matches metadata and heading chunks while the concept remains the lifecycle unit.
- The knowledge workflow runs focused tests, strict-profile validation, graph inspection, provenance/freshness/source-drift diagnostics, two builds, and a recursive byte comparison in CI.
- Generated knowledge remains local and repository-visible. CI does not upload artifacts, no public deployment is configured, and no verbatim bundle archive is published because a complete source-and-license review has not been recorded.
- The publication decision and substantively updated current-status records remain `draft` until human review; historical verification is retained where applicable.

Result: complete. The bundle has deterministic machine outputs and filterable local retrieval, while publication scope and archive safety are explicit rather than inferred.
...
| `OKF2000`–`OKF2999` | strict profile fields, controlled types, tags, and body contracts |
...
- missing strict-profile `title`, `description`, `status`, `generated`, or domain tag;
...
Strict strict-profile checks must be selectable separately from base OKF conformance so an externally supplied conformant bundle is not rejected for lacking Rocci's optional conventions.
...
| 10 | Publication | Keep rendered knowledge local/repository-visible through bootstrap; do not publish a verbatim archive until every included source and license is reviewed | Confirmed 2026-08-16 |
...
- `knowledge/` passes base OKF v0.2 conformance and the documented strict profile;
